...
contract Delegates{
 ...
 // uint constant Yy = 1801119347122147381158502909947365828020117721497557484744596940174906898953;
  ...
  function GenCommit(string seed) public returns (bool){
       for(uint j=0; j < delegates.length; j++){
        ...
        for(uint i=0; i< 10; i++){
            if(i==0){
                delegate_info[delegates[j]].r[i]= uint(sha256(i,j, seed));
                 delegate_info[delegates[j]].comm[i]=uint(sha256(delegate_info[delegates[j]].Y,delegate_info[delegates[j]].r[i]));
            }
            else{
        delegate_info[delegates[j]].r[i]= uint(sha256(i,j, seed));
        delegate_info[delegates[j]].comm[i]=uint(sha256(delegate_info[delegates[j]].comm[i-1],delegate_info[delegates[j]].r[i]));
        }
        }
      }
      return true;
 ...
 function delegateInfoAccess(address _delegate) public returns (uint[2],uint[], uint,uint[2],uint[],uint[],uint[2][])
  {
      return (delegate_info[_delegate].pubkey,
      delegate_info[_delegate].comm,
      delegate_info[_delegate].X,
      delegate_info[_delegate].Y,
      delegate_info[_delegate].r,
      delegate_info[_delegate].coef,
      delegate_info[_delegate].genshares);
  }
