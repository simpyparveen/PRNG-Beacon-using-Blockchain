...
library ECCMath {
    ...
    /// @return (Px", Py", 1)
    function toZ1(uint[3] P, uint zInv, uint z2Inv, uint prime) internal constant returns (uint[2] a_p){
    ...
    /// @param prime The prime modulus.
    /// @return (Px", Py", 1)
    function toZ1(uint[3] PJ, uint prime) internal constant returns(uint[2] a_p) {
...
library Secp256k1 {
    ...
    function isPubKey(uint[2] P) internal constant returns (bool isPK) {
        isPK = onCurve(P);
    }

    /// @dev See Curve.isPubKey
    // TODO: We assume we are given affine co-ordinates for now
    function isPubKey(uint[3] P) internal constant returns (bool isPK) {
        uint[2] memory a_P;
        a_P[0] = P[0];
        a_P[1] = P[1];
        isPK = onCurve(a_P);
    ...
    // outData: Rx, Ry, Rz
    function _add(uint[3] P, uint[3] Q) internal constant returns (uint[3] R) {
        if(P[2] == 0)
            return Q;
        if(Q[2] == 0)
            return P;
    ...
    // outData: Rx, Ry, Rz
    function _addMixed(uint[3] P, uint[2] Q) internal constant returns (uint[3] R) {
        if(P[2] == 0)
            return [Q[0], Q[1], 1];
        if(Q[1] == 0)
            return P;
    ...
    function _addMixedM(uint[3] P, uint[2] Q) internal constant {
        if(P[1] == 0) {
            P[0] = Q[0];
            P[1] = Q[1];
            P[2] = 1;
    ...
    function _double(uint[3] P) internal constant returns (uint[3] Q) {
        uint p = pp;
        if (P[2] == 0)
            return;
        uint Px = P[0];
    ...
    function _doubleM(uint[3] P) internal constant {
        uint p = pp;
        if (P[2] == 0)
            return;
        uint Px = P[0];
    ...
    // Params: d, Px, Py
    // Output: Jacobian Q
    function _mul(uint d, uint[2] P) internal constant returns (uint[3] Q) {
        uint p = pp;
        if (d == 0) // TODO
            return;
