...
contract RNG{
    ...
    enum State { SETUP, COMMITMENT, OPENING, DISTRIBUTION, VALIDATION,REVEALING, RECONSTRUCTION, FINISHED, FAILED }
  State public state;
  ...
  uint[] public x;
  uint blockhash;
  uint public beacon=uint(sha256("Our scheme initialized"));
  uint count=0;
  uint Threshold;
  uint firstcome;
  uint e;
...
uint[]  pos;
uint[] public Q;
mapping(uint=>address) public Election;
...
address owner; // i.e. the Random Contract
  ...
  function OurScheme(uint64 _number,uint _th, uint _csize) public{
    owner = msg.sender;
    Threshold=_th;
    ComSize=_csize;
 ...
 function CommitteeElection() returns (bool) {

   for(uint i=0; i< delegates.length; i++){
       Q[i]=uint(sha256(beacon,delegates[i]));
       Election[Q[i]]=delegates[i];
   }
   Sort();
   for(uint j=0; j < ComSize; j++){
       committee.push(Election[Q[j]]);
       comhas[Election[Q[j]]]=true;
   }
 ...
 function Sort() public {
      //bubble sort technique to sort the leaders in leader election then we select winner with lowest hash
      for (uint i = 0; i < delegates.length; i++)
      {
      // Last i elements are already in place
      for (uint j = 0; j < delegates.length-i-1; j++)
        {
            if (Q[j] > Q[j+1])
              (Q[j],Q[j+1]) = (Q[j+1],Q[j]); //Swapping
        }
      }
  }
 ...
 function OpeningCom(uint[2] _y, uint256 _r) public returns (bool){
     if(state==State.OPENING && comhas[msg.sender]){
 ...
 function Distribution(uint[2][] _commcoef) public returns (bool){
     if(state==State.DISTRIBUTION && comhas[msg.sender]){
 ...
 function RecieveSecrets(uint _x) public returns(bool){
     if(state==State.REVEALING && comhas[msg.sender]){
         uint[2] memory temp;
         uint sumx;
         count=count+1;
         temp=ECCMath.toZ1(Secp256k1._mul(_x,G),nn);
         if(temp[0]==delegate_info[msg.sender].y[0] && temp[1]==delegate_info[msg.sender].y[1] ){
         x[indexes[msg.sender]]=_x;
         }
         if (firstcome==0)
     {
        uint256 T=Time_call();
     ...
     if (Time_call()-T >= RoundTime){
         for(uint j=0; j<x.length; j++){
             if(x[indexes[committee[j]]]==0){
                 abort.push(committee[j]);
             }
         }
         if(abort.length==0){
  ...
  function ReceiveShares(uint[2][] _share) public returns (bool){
     ...
     count=count+1;
     for(uint ii=0;ii < abort.length; ii++){
         delegate_info[msg.sender].shares[indexes[abort[ii]]]=_share[ii];
     }
     ...
     if (Time_call()-T >= RoundTime){
         for(uint j=0; j < abort.length; j++){
         Reconstruction(indexes[abort[j]]);
         }
         uint sumx;
         for(uint kk=0; kk< committee.length; kk++){
